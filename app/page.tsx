...
export default async function HomePage() {
  ...
  return (
    <div style={{ maxWidth: '1250px', margin: '0 auto', padding: '40px 20px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(400px, 1fr))', gap: '40px' }}>
      <section>
        <h2 style={headerStyle}>📅 Plánované akce</h2>
      ...
      <section>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
          ...
          <Link href="/vysledky" style={{ color: '#fbbf24', fontSize: '0.85rem', textDecoration: 'none', fontWeight: 'bold' }}>Kompletni vysledky -{'>'}</Link>
        </div>
        {categories?.map(cat => {
...
const glassCardStyle: any = { background: 'rgba(12, 12, 12, 0.8)', backdropFilter: 'blur(15px)', borderRadius: '24px', border: '1px solid rgba(255,255,255,0.1)', overflow: 'hidden' };
const headerStyle: any = { fontSize: '1.6rem', fontWeight: '900', textTransform: 'uppercase', margin: '0 0 20px 0', color: '#fff' };
